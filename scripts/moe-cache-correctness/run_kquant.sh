...
if [ ! -f "$MODEL" ]; then
  ...
  python3 "$HERE/make_tiny_moe.py" --n-embd 256 --n-ff 256 "$F32" || exit 1
...
echo "$CACHE"
echo "== cache + hybrid, tight budget (forces staging skips) =="
...
[ "$BASE" = "$HYB" ]   || { echo "FAIL: hybrid output diverges from baseline"; FAIL=1; }
[ "$FAIL" = 0 ] && echo "PASS: token-identical across baseline/cache/hybrid"
exit $FAIL
